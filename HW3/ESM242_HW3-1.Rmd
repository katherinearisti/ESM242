...
out_tree1a=nloptr(x0=10, #not sure if this is the correct x0
          ...
          lb=0,
          opts = options,
          discount=1/1.05,a=10,b=53.27,c=250,p=1.5)
...
obj_fcn_t6 <- function(choice,a,b,c,p,discount){
  ...
  benefits <- vector(mode="numeric", length = 0)
   ...
   Q=exp(a-b/choice)
  state[1] = choice[1]
  benefits[1] = (-c + p * Q * exp(-discount*state[1]) - c * exp(-discount*state[1]) + p * Q * exp(-discount*state[1]) - c * exp(-discount*state[1]))
  
  
  for (i in 2:6){
     Q=exp(a-b/choice)
  state[i] = choice[i] + state[i-1]
  benefits[i] = (-c + p * Q * exp(-discount*state[i]) - c * exp(-discount*state[i]) + p * Q * exp(-discount*state[i]) - c * exp(-discount*state[i]))
  
  return(-benefits)
    }

  #choice = seq(from=0, time=6)
  #Q=exp(a-b/choice)
...
out_tree1b=nloptr(x0=10, #not sure if this is the correct x0
          eval_f=obj_fcn_t6,
          lb=0,
          opts = options,
          discount=1/1.05,a=10,b=53.27,c=250,p=1.5)
