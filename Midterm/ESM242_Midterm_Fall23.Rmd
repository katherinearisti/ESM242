...
```{r}
period=10
obj_stock_func <- function(choice, Z0, p, a, b, gamma, alpha, discount, period){ #took out Z?
  
  state<-vector(mode="numeric",length=0)
  benefits<-vector(mode="numeric",length=0)
  
  state[1] = Z0 #would initial state be Z0?
...
constraint_function <- function(choice, Z0, p, a, b, gamma, alpha, discount, period){
