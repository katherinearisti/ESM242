...
```{r}
period=11
obj_stock_func <- function(choice, int_state, p, a, b, gamma, alpha, discount){ #took out Z?
...
constraint_function <- function(choice, p, a, b, gamma, alpha, discount, int_state){
  
  #state <-vector(mode="numeric",length=10)
  #const <-vector(mode="numeric",length=10)
...
```{r}
stock_2a <- function(choice, int_state=40, period=10, a=2, b=-0.25, p=100, alpha=2, gamma=0.2, discount=1/1.04){
  ...
  pv=discount^t*benefits
  npv=sum(pv)
  data.frame("Period"=t, "PVNB"=npv, "Stock Pollution"=state, "Rate of Production of Pollutant"=choice)
  
}

out_2a$solution[11] = NA

tbl_2a=round(stock_2a(out_2a$solution),3)    
