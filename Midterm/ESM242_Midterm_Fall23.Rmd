...
```{r}
obj_stock_func <- function(choice, int_state, p, a, b, gamma, alpha, rho){ #took out Z?
  
  state<-vector(mode="numeric",length=0)
  benefits<-vector(mode="numeric",length=0)
  
  state[1] = int_state #would initial state be Z0?
  benefits[1] = rho^t(p*choice[1] - (a*state[1] - b*state[1]^2)) #confused on which is state and which is choice, is state z and choice q? also confused on how to implement Z0 and Zt
  ...
  #t = seq(from=0, 10)
  #confused on what this would be or is rho discount?
  npv= sum(benefits)
  return(-npv)
  
}

constraint_function <- function(choice, int_state, p, a, b, gamma, alpha, rho){
...
out_2a=nloptr(x0 = rep(0, times=10),
              int_state = 40, 
