...
exhaust <- function( T, a, b, delta, R0){
  ...
  p <- vector(mode="numeric", length = length(t))
  q <- vector(mode="numeric", length = length(t))
  R <- vector(mode="numeric", length = length(t))
  pi <- vector(mode="numeric", length = length(t))
  ...
  for(k in 2:(T+1)){
    ...
    R[k] = R[k-1] 
    pi[k] = 1/(1+delta) * p[k] * q[k]
    
    return(data.frame("Price" = a, "Time" = T, "Profits" = q, "Reserves" = R)) #t, p, q, R, and pi
  }
  
}

table1a <- exhaust(T= 1, a= 40, b= 1, delta = 0.05, R0= 778.1259)
